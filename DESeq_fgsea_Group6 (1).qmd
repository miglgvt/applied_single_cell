...
Identify the top 20 differentially expressed genes in Breast Cancer cell 3 hours after stimulation, compared to unstimulated cells. Sort the genes by padj
...
```{r}
# Filter for the gene GREB1 to label it on the plot
label_data <- res_tbl %>%
  dplyr::filter(gene == "GREB1" | gene== "TAPBP" | gene == "IFITM3" | gene== "NR2F1" | gene == "MALRD1" | gene == "PRKD1")
# Update the significance categories for coloring
res_tbl <- res_tbl %>%
  mutate(
    significant = case_when(
      padj < 0.005 & log2FoldChange >= 2 ~ "Upregulated",
      padj < 0.005 & log2FoldChange <= -2 ~ "Downregulated",
...
volcano <- ggplot(res_tbl, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(color = significant), alpha = 0.4, size = 1) +
  scale_color_manual(values = c(
    "Not Significant" = "grey", 
    "Upregulated" = "#4169E1", 
    "Downregulated" = "#BC3F3F"
  )) +
  labs(
  ...
  geom_text(data = label_data, aes(label = gene), 
          vjust = -1, color = "black", size = 2, fontface = "bold") +
  theme_minimal() +
...
volcano <- volcano +
  ...
  geom_hline(yintercept = -log10(0.005), linetype = "dashed", color = "black")
