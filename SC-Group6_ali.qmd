...
I will first try to create a merged object and integrate it:

```{r}
merged.seu <- merge(seurat_list_qc$PT1, y = c(seurat_list_qc$PT2, 
                                                 seurat_list_qc$RT1,
                                                 seurat_list_qc$RT2 ,
                                              seurat_list_qc$NT1,
                                              seurat_list_qc$NT2), 
                       add.cell.ids = c("PT1", "PT2", "RT1", "RT2", "NT1", "NT2"), 
                       project = "scRNA_merged")
```

Check if the data is split. If so, continue
...
```{r}
merged.seu <- FindNeighbors(merged.seu, dims = 1:30, reduction = "pca")

```

```{r}

merged.seu <- FindClusters(merged.seu, resolution = 0.5 , cluster.name = "unintegrated_clusters")

merged.seu <- RunUMAP(merged.seu, dims = 1:30, reduction = "pca", reduction.name = "umap.unintegrated")

DimPlot(merged.seu, reduction = "umap.unintegrated", group.by = c("orig.ident", "seurat_clusters"))
```

```{r}
integrated.seu <- merged.seu

integrated.seu[["RNA"]] <- split(integrated.seu[["RNA"]], f = integrated.seu@meta.data$orig.ident)

integrated.seu <- NormalizeData(integrated.seu)
integrated.seu <- FindVariableFeatures(integrated.seu)
integrated.seu <- ScaleData(integrated.seu)
integrated.seu <- RunPCA(integrated.seu)

integrated.seu <- IntegrateLayers(object = integrated.seu, method = CCAIntegration, orig.reduction = "pca", new.reduction = "integrated.cca",
    verbose = FALSE)

# re-join layers after integration
integrated.seu[["RNA"]] <- JoinLayers(integrated.seu[["RNA"]])

integrated.seu <- FindNeighbors(integrated.seu, reduction = "integrated.cca", dims = 1:30)

```

```{r}
integrated.seu <- FindClusters(integrated.seu, resolution = 0.5)
integrated.seu <- RunUMAP(integrated.seu, dims = 1:30, reduction = "integrated.cca")
DimPlot(integrated.seu, reduction = "umap", group.by = c("orig.ident", "seurat_clusters"))
```

```{r}
saveRDS(integrated.seu, file = "data/scRNA/integratedTT_NT_12.rds")
...
# seurat_list_qc$PT3,
# seurat_list_qc$RT3), add.cell.ids = c("PT1", "PT2", "RT1", "RT2", "RT3", "PT3"),                       project = "scRNA_TTs_unintegrated")
